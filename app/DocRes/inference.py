...
def deblurring(model,im_path):
    ...
    im_org = cv2.resize(im_org, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA)
    ...
    out_im = cv2.resize(out_im, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
...
def binarization(model,im_path):
    ...
    if max(width, height) > MAX_SIZE:
        ...
        new_widht = int(width * scale)
        new_height = int(height * scale)
        im_org = cv2.resize(im_org, (new_widht, new_height))
    #im_org = cv2.resize(im_org, None, fx=0.5, fy=0.5, interpolation=cv2.INTER_AREA)
    ...
    #out_im = cv2.resize(out_im, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
    out_im = cv2.resize(out_im, (width,height))
...
def inference_one_im(model,im_path,task):
    if task=='dewarping':
        ...
        prompt1,prompt2,prompt3,restorted = dewarping(model,im_path)
        cv2.imwrite('restored/step1.jpg', restorted)
        prompt1,prompt2,prompt3,restorted = deshadowing(model, 'restored/step1.jpg')
        cv2.imwrite('restored/step2.jpg', restorted)
        prompt1,prompt2,prompt3,restorted = appearance(model, 'restored/step2.jpg')
        cv2.imwrite('restored/step3.jpg', restorted)
        prompt1, prompt2, prompt3, restorted = binarization(model, 'restored/step3.jpg')
        cv2.imwrite('restored/current_check.jpg', restorted)
        # os.remove('restorted/step1.jpg')
        # os.remove('restorted/step2.jpg')

    return prompt1,prompt2,prompt3,restorted

def image_restore(im_path, model_path):

    model = model_init(model_path, load_from_file=True)
    ...
    cv2.imwrite("../OUTPUT/IMG_restored.png", restorted)
